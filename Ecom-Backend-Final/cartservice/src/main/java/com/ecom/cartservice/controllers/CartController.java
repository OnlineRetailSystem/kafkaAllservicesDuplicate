...
import com.ecom.cartservice.kafka.CartEventProducer;
...
public class CartController {
    ...
    public ResponseEntity<?> addToCart(@RequestBody CartItem item) {
        ...
        // Publish Kafka event
        cartEventProducer.publishItemAddedToCart(
            item.getUsername(), 
            item.getProductId(), 
            item.getProductName(), 
            item.getQuantity(), 
    ...
    public ResponseEntity<CartItem> updateCartItem(@PathVariable Long id, @RequestBody CartItem updatedItem) {
        return cartRepository.findById(id)
                .map(item -> {
                    ...
                    // Publish Kafka event
                    cartEventProducer.publishCartUpdated(
                        item.getUsername(), 
                        item.getProductId(), 
                        item.getProductName(), 
                        item.getQuantity(), 
    ...
    public ResponseEntity<Object> removeCartItem(@PathVariable Long id) {
        return cartRepository.findById(id)
                .map(item -> {
                    ...
                    cartEventProducer.publishItemRemovedFromCart(
                        item.getUsername(), 
                        item.getProductId(), 
                        item.getProductName(), 
                        item.getQuantity(), 
    ...
    public ResponseEntity<Void> clearCart(@PathVariable String username) {
        // Publish Kafka event before clearing
        cartEventProducer.publishCartCleared(username);
